"""Stage 2: DCFG Preprocessor - expands macros in .dcfg files before YAML parsing."""
...
class DCFGPreprocessorError(Exception):
    """Raised when macro preprocessing fails."""
    pass


def expand_macros(dcfg_text: str, class_names: list[str] = None, function_names: list[str] = None) -> str:
    """Expand macros in DCFG text before YAML parsing.

    Args:
        dcfg_text: Raw DCFG text with potential macros.
    ...
    Raises:
        DCFGPreprocessorError: If an unknown macro is encountered.
    """
...
def validate_no_unknown_macros(text: str) -> None:
    ...
    Args:
        text: The DCFG text to check.

    Raises:
        DCFGPreprocessorError: If an unknown macro is found.
    """
    ...
    for match in re.finditer(all_macro_pattern, text):
        ...
        if macro_name not in known_macros:
            raise DCFGPreprocessorError(
                f"Unknown macro '___{macro_name}___'. Valid macros are: __CLASSES__, __FUNCTIONS__"
            )
